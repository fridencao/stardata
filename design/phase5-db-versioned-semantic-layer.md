...
| 3 | 「预览」按钮 | `PreviewDraft` 复用同一渲染+解析管线但不落库；`PreviewSemanticProject` RPC + 前端「预览校验」按钮 | ✅ |
...
**5.2 端到端流程**：草稿编辑 → 预览校验 → 发布（dry-run 门控）→ 资源可见性开关 → 运行时重载 → 业务侧可见。全链路打通。
