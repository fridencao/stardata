...
// PublishGatePath is the project-relative file the runtime's publish gate reads.
// StarData serves it as a virtual file for DB-mode projects so the existing
// runtime gate (which reads /publish.yaml) needs no change.
const PublishGatePath = "publish.yaml"

// RenderPublishGate turns the set of business-visible resource names into the
// publish.yaml body the runtime already knows how to parse. Emitting the file at
// all makes the gate fail-closed: the runtime treats a present publish.yaml as
// "only these names are visible", so any resource not listed here is hidden.
func RenderPublishGate(visibleNames []string) []byte {
	var b strings.Builder
	b.WriteString("# Generated by StarData from resource_visibility. Do not edit.\n")
	b.WriteString("published:\n")
	for _, n := range visibleNames {
		// Quote to survive names that look like YAML scalars (numbers, bools).
		b.WriteString("  - ")
		b.WriteString(quoteYAMLScalar(n))
		b.WriteString("\n")
	}
...
func quoteYAMLScalar(s string) string {
	// A double-quoted scalar with escaped quotes/backslashes is always safe.
