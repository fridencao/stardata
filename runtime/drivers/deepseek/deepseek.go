...
func init() {
	drivers.Register("deepseek", driver{})
	drivers.RegisterAsConnector("deepseek", driver{})
}

var spec = drivers.Spec{
	DisplayName: "DeepSeek",
	Description: "Connect to DeepSeek's OpenAI-compatible API for language models.",
	...
	ConfigProperties: []*drivers.PropertySpec{
		...
		},
		{
			...
			Description: "The base URL for the DeepSeek API (defaults to 'https://api.deepseek.com/v1').",
			Placeholder: "",
		},
		{
			Key:         "api_type",
...
func (d driver) Open(_, instanceID string, config map[string]any, st *storage.Client, ac *activity.Client, logger *zap.Logger) (drivers.Handle, error) {
	...
	default:
		...
		if conf.BaseURL != "" {
			...
			opts = append(opts, option.WithBaseURL("https://api.deepseek.com/v1"))
		}
	}
