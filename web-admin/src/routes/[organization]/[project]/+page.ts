...
export const load = async ({ parent, params: { organization, project }, url }) => {
  ...
  // their own entry — the Studio workbench at `/studio/[domain]` — so bounce
  // them there instead of showing the business-facing home. They can still
  // reach chat/boards via the portal nav (those are separate routes,
  ...
  if (projectPermissions?.manageProject && !url.searchParams.has("preview")) {
    throw redirect(307, `/studio/${project}`);
  }
};
