<script lang="ts">
  ...
  // still initializing. Because Studio mounts the edit-session shell, any
  // edit-session failure bubbles up as an unhandled error. This boundary
  // translates the raw error into user-friendly language for technical governors.
  $: error = $page.error;
  $: studioBase = `/studio/${$page.params.domain}`;
