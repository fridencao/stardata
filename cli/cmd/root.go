...
For Agents:
If you are administrating a StarData Cloud project, the CLI help menus speak for themselves (e.g. "stardata project -h" or "stardata user -h").
But if you are developing a StarData project, it is imperative that you initialize and load the StarData agent instructions in the project directory.
...
func Run(ctx context.Context, ver version.Version) {
	...
	// Crude check to detect if we may be running as StarData Cloud, namely if the command is `stardata admin ...` or `stardata runtime ...`.
...
func RootCmd(ch *cmdutil.Helper) *cobra.Command {
	...
	long := "Work with StarData projects from the command line."
	if !ch.Interactive {
		long += "\n\n"
...
func HandleExecuteError(ch *cmdutil.Helper, err error) int {
	...
	if strings.Contains(errMsg, "org not found") {
		...
		ch.Println("Please log in or sign up for StarData with `stardata login`.")
	} else {
