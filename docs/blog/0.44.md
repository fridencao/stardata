...
# StarData 0.44 - New visualization types, File Explorer, Pinot Connector, new installation scripts
...
## File Explorer
As both StarData grows and your project grows we have introduced a new way of navigating your project to support both the product growth and your project growth. This is the first release of many with improvements to StarData Developer so stay tuned for future updates!

As we add more and more resource types into StarData to support new functionality it started to be a problem with having fixed sections for certain types. So instead of having fixed listing of files we are now displaying the full project listing in a more traditional file and folder explorer view.
...
Our new installation scripts allows you to install StarData without having to utilize sudo privileges.  
The new installation script will allow you to install StarData into a local path with optional addition to your local paths.  

If you previously had StarData installed, please uninstall StarData before trying out the new install script if you want to see the new features (Don't worry, it's not mandatory!)  
To uninstall run: `curl https://rill.sh | sh -s -- --uninstall` 


## Bug Fixes and Misc
- All newly deployed projects on StarData Cloud will now use a single directory for all instance data. (*Note: This change will result in an one-time reset of all existing deployed projects but should otherwise not impact production!*)
...
- Non-admin users should now be able to remove themselves from organizations and projects on StarData Cloud.
...
- Fixed bug where `stardata-QTD` would not always populate correctly when specified in `available_time_ranges` for a dashboard.
...
- Added SAML SSO as an available login option when logging into StarData Cloud.
...
- The `type` property should now automatically be included as part of the YAML config of any new resource created in StarData (e.g. sources, dashboards, etc.)
- Removed sources and models as available resources that can be added in StarData Developer when using either ClickHouse or Druid as the OLAP engine.
...
- Improved the Table Explorer UX within StarData Developer so that clicking on the table entry will now show / hide the table's schema.
...
- Added more consistency to the error page being shown across StarData Cloud and StarData Developer.
- SQL files will now automatically be considered to be model resource types by default within StarData Developer.
...
- Improved hardening of StarData to better handle edge cases around renaming resources and objects.
