...
# StarData 0.43 - Slack Alerts, automated repo creation and exports!
...
Tired of manually creating the Github repository and having to sync it to a StarData project _before_ deploying to StarData Cloud? We've made this process easier for new and experienced users alike. StarData Cloud can now automatically create a Github repository on your behalf for new StarData projects that you wish to deploy. Let us know what you think!
...
## Bug Fixes and Misc
- **Note:** For users of the MotherDuck connector, please note that MotherDuck still only supports up to DuckDB v0.9.2 so it is recommended to stay on StarData v0.42 until support has been updated by the MotherDuck team.
- Upgraded the underlying version of DuckDB used in StarData to the latest `v0.10.1`.
...
- Fixed bug where admins could not properly use the "View As" dropdown in StarData Cloud.
...
- Improved the styling of the nav bar in StarData.
- StarData Developer will now hide the `Edit model` menu item when there is no actual underlying model to edit (i.e. dashboard is built directly on top of a Source or Table resource)
- Added guardrails to prevent fetching of bookmarks for anonymous users on shared dashboards.
- Added UX improvements to the default StarData theme.
...
- Added hardening to source navigation to improve consistency when navigating from / to sources in StarData Developer.
- Added shortcut to reports and alerts from the avatar dropdown when viewing a dashboard.
- Added a warning indicator in StarData Developer to call out unsupported data types in the UI when listing tables (rather than failing to load the entire table).
- Improved the color schema for categorical charts in StarData.
...
- Updated the Druid driver in StarData to leverage the [Druid SQL API](https://druid.apache.org/docs/latest/api-reference/sql-api) instead.
