...
# StarData 0.50 - UI for editing measures and dimensions, metrics layer and StarData Cloud settings.
...
We have introduced a visual way to create and edit metrics and dimensions in StarData Developer. With just a few clicks you can now add or edit measures and dimensions without having to write a single line of code! We are super excited to bring measure and dimesions creation to users that aren't familiar with yaml and we can't wait to extend the same editing experience to StarData Cloud.
Don't worry, yaml and code artifacts aren't going away and you can easily toggle between code and UI.

## Metrics Layer

Your dashboards has always contained definitions for dimensions and measures, as StarData continues to grow we have seperated the metrics view from visual dashboard config. This comes with a lot of benefits such as being able to have multiple dashboards reference the same metrics view. In addition it will make the semantics of StarData easier to understand as APIs and other features now references a metrics view rather then a dashboard. Your old dashboard files will continue to work but it's encouraged to migrate at your earliest convenience. 

## Settings page

You can now manage your StarData Cloud settings, such as name and allow list domains, directly from within the StarData Cloud UI. More settings to come!
...
- new feature in the StarData Developer UI for incremental models and splits, allows for incremental refreshes and insight into splits.
- `stardata deploy` command changed to `stardata project deploy` and `stardata project connect-github`, `stardata deploy` will still work but is a wrapper of the latter two.
- upgrade to duckdb 1.1.0
- new  icons for the various StarData components! 
...
- improved response handler which fixes a few issues including StarData Developer not adding/removing connectors immediately.
- improved idle detection in StarData Developer. 
- improved insighting into model splits.
- improved UI: after inviting users `skip` to `continue`.
- improved UI: missing bottom border on StarData Cloud on certain pages.
