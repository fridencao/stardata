...
# StarData 0.46 - Comparisons everywhere, more improvements to StarData Developer and external OLAP engines.
...
Now you can connect to your own OLAP engine, such as ClickHouse, Druid or Pinot, without having to restart StarData or passing in connection strings as startup parameters. It's as easy as selecting ClickHouse in the Connect dialog and off you go!
...
- Added more consistency to how page titles are rendered in StarData Developer.
...
- StarData now automatically embeds all necessary DuckDB extensions in its installation binary.
- Refactored the Table Explorer to be connector-specific.
- Fixed issue where StarData Developer could hang in specific scenarios when multiple tabs in the browser are open and StarData is attempting to reconcile.
- Added more consistency by skipping invalidation if no filename is present.
- StarData will now automatically set the session timezone as UTC when using the ClickHouse connector.
...
- Added backend improvements to remove unnecessary and/or redundant network requests in StarData Developer.
- Fixed issue where comparison visualizations do not always work properly when using ClickHouse-backed dashboards.
- Improved error handling for reconciling resources in StarData.
- StarData CLI will no longer show the "pending user invites" header if there aren't any outstanding pending user invites.
- Improved how project-level members are listed from within the StarData CLI.
...
- Fixed issue where StarData Developer would sometimes show a "source imported successfully" message before the ingestion was actually complete.
- StarData will now automatically more explicitly declare required connector properties when setting up a new connector.
- StarData now supports setting individual connection properties in our OLAP connectors (ClickHouse, Druid, Pinot).
...
- StarData Developer will now represent databases and database schemas in an expandable or collapsible folder structure in the Table Explorer.
...
- Fixed issue where editing either the `rill.yaml` or `.env` files could cause StarData Developer to become unresponsive.
- StarData will no longer generate an empty `source` and `model` directory by default for newly initialized StarData projects.
- StarData will now utilize system tables instead of `information_schema` for ClickHouse OLAP queries.
