...
# StarData 0.60 - Multiple metrics in leaderboards and leaderboards in Canvas!
...
- Added metrics view and dashboards to `stardata project refresh` so external tables also refreshed.
...
- Fixed issue when up to date GitHub Repository failed to connect to StarData Cloud project. 
- Fixed bug in new filter when newline / comma present.
- Fixed issue where Google Drive backed Big Query Table not able to load in StarData.
