...
# StarData 0.84 - Canvas Public URLs, Delta Lake & Iceberg Connectors, Dynamic Y-Axis Scale
...
StarData now supports connecting to Delta Lake and Apache Iceberg tables. Bring your table format data directly into StarData for modeling and dashboarding with full documentation and test coverage.


## Interactive `stardata init`

`stardata init` is now interactive by default, guiding you through project setup step by step.
```bash
stardata init
...
- Fixed app crash when stardata time syntax is used in comparison
...
- Fixed 'as of' part from stardata time not being honoured in the UI
...
- Fixed `stardata upgrade` interactivity
