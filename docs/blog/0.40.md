...
# StarData 0.40 - Measure filters, MySQL connector, exports, branding updates and more!
...
- Reduced warnings about "broken rill.yaml" files that can occur with empty StarData projects.
...
- Added support for a native Salesforce connector in StarData (original community contribution made by our friend Christian!).
...
- Fixed bug in which `stardata start` fails to start up when a proxy server is configured because an invalid request is sent to the proxy (commmunity contribution by Christian!).
