...
# StarData 0.64 - Export metadata headers, search in Pivot pills, User groups when sharing and more
...
- Added new CLI commands for user access tokens. `stardata token ...`.
- Added hover intent logic to Cloud dashboard's display tooltip.
- Added `stardata project clone` command. 
...
- Fixed issue in StarData Developer where inspector would not show for partially loaded incremental models. 
