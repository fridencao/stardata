...
# StarData 0.4 – Faster install, numeric viz improvements

:::note
⚡ StarData Developer is a tool that makes it effortless to transform your datasets with SQL. These are our release notes for our `0.4.0` release, still in Tech Preview.
...
We don’t want the little things to get in the way of sharing the power of StarData’s data exploration with you. With this release we focused on making installing StarData faster and easier so you can get out of installer dialogues and back to exploring your data.
- **Faster install times —** We have used `vercel/pkg` to build binaries for different OSes and architectures. This will bundle the StarData Developer application as a single file, including a build of DuckDB, to make it faster than ever to get started. We currently support installing the binary through asset download links in our Github release change log.
