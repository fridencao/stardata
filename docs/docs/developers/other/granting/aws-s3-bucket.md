...
* Create a new bucket, in the US Standard S3 region, named `stardata-ORG_NAME-share`, where **"ORG_NAME"** is your organizational name. Make sure the bucket name is a DNS-compliant address. For example, the name should not include any underscores; use hyphens instead, as shown in the example above.
...
We can provide access to the desired buckets using the bucket policies. You can provide access to the root user for StarData AWS Account. 
...
1. Add the following bucket policy for the access: (Replace stardata-ORG_NAME-share or bucket-to-be-shared with the bucket name)
...
```shell
export BUCKET="stardata-ORG_NAME-share" #Create a bucket with Name of Bucket to be shared
...
We can provide access to the S3 Bucket through an IAM Role which will be assumed by the StarData AWS Account to gain the access to the buckets. 
...
Description: 'AWS CloudFormation Template for providing StarData Access to S3 Bucket. It creates a
...
As part of your data uploads, StarData requires logging to be enabled for the data you intend on sending to us for processing. StarData may use these logs for troubleshooting, pipeline optimization, and/or billing. In order to allow StarData to access these logs, please follow the instructions below to set up the proper configuration.

If you have not enabled logging for the bucket StarData is ingesting data from, please follow these steps:

1. Go to http://aws.amazon.com
    * Login to your AWS account on the AWS Management Console and navigate to the S3 tab
    * Click the “Properties” icon on the upper-right menu to bring up the “Properties” options on the bucket StarData is ingesting data from
    * Under “Logging”, click “Enabled”
    * Under “Target Bucket”, select the same name of the bucket StarData is processing data from
...
3. Click on "Create Stack" and under "Choose a template", either upload the template provided by StarData, or use the following s3URL: https://s3.amazonaws.com//cf-templates.rilldata.com/RillS3LogRole.yaml

4. The template allows you to set the following parameters:
    * **"Stack name"**
      * Name the stack `stardata-ORG_NAME-s3Logs`
    * **"S3LogBucketName"**
...
6. Once the stack has finished being created, send your StarData representative the results on the "Outputs" tab.
    * **RoleARN**: This IAM role identifier allows StarData to ingest the S3 Logs
    * **S3BucketName**: This is the name of S3 Bucket that the StarData role can have access to, in this case, it should be the bucket name you inputted into Cloudformation
