...
description: Configuring credentials for your deployed project on StarData Cloud
...
We recommend reviewing the [Development/Production Setup](/developers/build/connectors/templating) documentation before deploying your project to StarData Cloud to ensure that your local development and production environments have been separated.

This will ensure that your shared dashboard will be decoupled from your local StarData Developer environment, and you can further develop your dashboards locally without worrying about data availability.

:::

When deploying a project, credentials that have been defined in your `.env` file will be automatically passed into your StarData Cloud project. However, for [remote sources](/developers/build/connectors) that are dynamically retrieving your credentials via the CLI, such as S3 and GCS, you will need to ensure that these are [defined in the .env file]( /guide/administration/project-settings/variables-and-credentials#credentials-naming-schema). 

[Local credentials](/developers/build/connectors/credentials#setting-credentials-for-stardata-developer) are used by StarData Developer to connect to sources from your local machine, while deployment credentials are used by StarData Cloud for production workloads. There are a [few ways to set up credentials in StarData Developer](/developers/build/connectors/credentials/#setting-credentials-for-stardata-developer); however, you will need to ensure that they are set up in your `.env` file for a seamless experience.
...
:::warning Be careful of overriding local credentials and/or pushing the wrong credentials to StarData Cloud

When using service accounts, it is very likely that different or even personal credentials are being used in local development (i.e., StarData Developer). Therefore, it is worth double-checking that the correct credentials are being used or set before [syncing credentials](/developers/build/connectors/credentials#pulling-credentials-and-variables-from-a-deployed-project-on-stardata-cloud) between your local instance of [StarData Developer and StarData Cloud](/developers/deploy/cloud-vs-developer) using the `stardata env push` and `stardata env pull` commands respectively.
