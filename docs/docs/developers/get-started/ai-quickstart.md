---
title: "Building StarData Projects with AI"
sidebar_label: "Agentic Quickstart"
sidebar_position: 5
---

StarData projects are defined as YAML and SQL files, which makes them a natural fit for AI coding agents. This guide walks through using an AI agent like Claude Code or Cursor to build a StarData project from scratch.

## Prerequisites

- [StarData CLI installed](/developers/get-started/install)
...
Run `stardata init` to create a new project. The interactive setup will prompt you for a project name, OLAP engine, and agent instructions:

```bash
stardata init
? Project name my-stardata-project
...
- `.claude/CLAUDE.md` — entry point that tells Claude Code to load StarData development skills
- `.claude/skills/` — detailed instructions for each resource type (models, metrics views, dashboards, etc.)
- `.mcp.json` — connects Claude Code to StarData's local [MCP server](/guide/ai/mcp)
...
```bash
# Cursor rules
stardata init my-project --agent cursor

# Tool-agnostic AGENTS.md format
stardata init my-project --agent agentsmd

# All formats at once
stardata init my-project --agent all
```
:::

### Adding agent instructions to an existing project

If you already have a StarData project, you can add agent instructions without affecting your existing files:

```bash
stardata init ./my-existing-project --agent claude
```

## Step 2: Start StarData in preview mode

Launch StarData Developer in [preview mode](/developers/build/getting-started#preview-mode) to get a clean, dashboard-only view while your AI agent handles the code:
...
With StarData running, open your AI agent in the project directory and start building. Here are some examples of what you can ask:

### Connect a data source
> "Connect to the parquet file at `gs://rilldata-public/auction_data.parquet`"

The agent will create a source YAML file and StarData will automatically ingest the data.
...
### Iterate
The agent has full context on StarData's resource types and YAML schemas. It can fix errors, refactor models, add new measures, and restructure your project — just describe what you want.
...
- **[Deploy to StarData Cloud](/developers/deploy/deploy-dashboard)** — share your dashboards with your team
- **[AI Chat](/guide/ai/ai-chat)** — ask questions about your data in natural language from StarData Cloud
- **[AI Configuration](/developers/build/ai-configuration)** — add `ai_instructions` to improve AI responses for your project
- **[StarData MCP Server](/guide/ai/mcp)** — connect Claude Desktop, ChatGPT, or other AI clients to StarData Cloud projects
