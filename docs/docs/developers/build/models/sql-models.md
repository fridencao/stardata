...
Add a new data model by either clicking 'model' in the 'Add' menu or selecting the '...' in any connector view or existing model. When you add a model, a code definition will automatically be created as a `<model_name>.sql` file in the `models` folder of your StarData project.

You can also create a model outside of the application and add it to StarData by placing a `<model_name>.sql` file in the `models` directory containing a DuckDB SQL `SELECT` statement. StarData will automatically detect and parse the model the next time you run `stardata start`.

## Annotating your models with properties

In most cases, objects are represented in StarData as YAML files. Models are unique in that any `<model>.sql` file can be considered a model resource in StarData, representing a SQL transformation that you would like to perform using a set of inputs and outputting a view or table (depending on the materialization type). For most other resources, available properties can be set directly via the corresponding YAML file. In the case of a model SQL file though, configurable properties should be set by annotating the top of the file using the following syntax:
...
By default, any new model that is created in a StarData project will populate a corresponding `.sql` file representing the model. Similarly, a `.sql` file that is directly created in the project directory will also be _automatically assumed_ by StarData to be a model by default. Therefore, it is not necessary to annotate the model resource with the `type` property.
...
- DuckDB doesn't support creating views based on `PIVOT` without an `IN` filter (and all models are materialized as views by default in StarData)
