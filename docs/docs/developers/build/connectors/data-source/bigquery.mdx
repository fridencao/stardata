...
[Google BigQuery](https://cloud.google.com/bigquery/docs) is a fully managed, serverless data warehouse that enables scalable and cost-effective analysis of large datasets using SQL-like queries. It supports a highly scalable and flexible architecture, allowing users to analyze large amounts of data in real time, making it suitable for BI/ML applications. StarData supports natively connecting to and reading from BigQuery as a source by leveraging the [BigQuery SDK](https://cloud.google.com/bigquery/docs/reference/libraries).

## Authentication Methods

To connect to Google BigQuery, you need to provide authentication credentials. StarData supports two methods:
...
Service Account JSON credentials provide the most secure and reliable authentication for BigQuery. This method works for both local development and StarData Cloud deployments.
...
For local development, you can use credentials from the Google Cloud CLI. This method is **not suitable for production** or StarData Cloud deployments.

:::warning Not recommended for production
Local Google Cloud CLI credentials only work for local development. If you deploy to StarData Cloud using this method, your dashboards will fail. Always use Service Account JSON for production deployments.
:::
...
When no explicit credentials are provided in the connector, StarData will automatically use your local Google Cloud CLI credentials. Then, [create your first model](#create-your-first-model).
