...
[Amazon Athena](https://docs.aws.amazon.com/athena/) is an interactive query service that makes it easy to analyze data directly in Amazon S3 using standard SQL. It is serverless, so there is no infrastructure to manage, and you pay only for the queries you run, making it cost-effective for a wide range of data analysis tasks. Athena is designed for quick, ad-hoc querying of large datasets, enabling businesses to easily integrate it into their analytics and business intelligence tools for immediate insights from their data stored in S3. StarData supports natively connecting to and reading from Athena as a source by leveraging the [AWS SDK for Go](https://aws.github.io/aws-sdk-go-v2/docs/).

## Authentication Methods

To connect to Amazon Athena, you need to provide authentication credentials. StarData supports two methods:
...
Access Key and Secret Key credentials provide the most reliable authentication for Athena. This method works for both local development and StarData Cloud deployments.
...
For local development, you can use credentials from the AWS CLI. This method is **not suitable for production** or StarData Cloud deployments.

:::warning Not recommended for production
Local AWS credentials only work for local development. If you deploy to StarData Cloud using this method, your dashboards will fail. Always use Access Key/Secret Key for production deployments.
:::
...
When no explicit credentials are provided in the connector, StarData will automatically use your local AWS CLI credentials. Then, [create your first model](#create-your-first-model).
