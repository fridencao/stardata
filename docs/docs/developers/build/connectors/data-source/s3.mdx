...
## Overview
[Amazon S3](https://docs.aws.amazon.com/AmazonS3/latest/userguide/Welcome.html) is a scalable, fully managed, and highly reliable object storage solution offered by AWS, designed to store and access data from anywhere in the world. It provides a secure and cost-effective way to store data, including common storage formats such as CSV and Parquet. StarData natively supports connecting to S3 using the provided [S3 URI](https://repost.aws/questions/QUFXlwQxxJQQyg9PMn2b6nTg/what-is-s3-uri-in-simple-storage-service) of your bucket to retrieve and read files.
...
When you add data from S3 through the StarData UI, you'll see two authentication options:
...
Access Key/Secret Key credentials provide reliable authentication for S3. This method works for both local development and StarData Cloud deployments.
...
StarData supports AWS IAM role assumption for enhanced security. This method allows StarData to temporarily assume an IAM role to access S3 resources. This method is only available through manual configuration.
...
For local development, you can use credentials from the AWS CLI. This method is **not suitable for production** or StarData Cloud deployments. This method is only available through manual configuration, and you don't need to create a connector file.
...
StarData will automatically detect and use your local AWS CLI credentials when no connector is specified.

:::warning
This method only works for local development. Deploying to StarData Cloud with this configuration will fail because the cloud environment doesn't have access to your local credentials. Always use Access Key/Secret Key or IAM Role Assumption for production deployments.
:::
...
2. Run the following command to create a new user (optionally replace `stardata-service-account` with a name of your choice):
    ```bash
    aws iam create-user --no-cli-pager --user-name stardata-service-account
    ```
...
3. Grant the user read access to data in S3:
    - To grant access to data in all buckets, run the following command:
        ...
        aws iam attach-user-policy \
            ...
            --user-name stardata-service-account
        ```
...
4. Run the following command to create an access key pair for the user:
    ```bash
    aws iam create-access-key --user-name stardata-service-account
    ```

5. Note down the `AccessKeyId` and `SecretAccessKey` values in the returned JSON object. Press "q" to exit the page.

### How to create an IAM role for cross-account access with StarData-provided AWS account

To set up an IAM role that grants StarData's AWS account access to your S3 buckets:
...
5. Enter StarData's AWS account ID that was provided to you by your StarData representative.

6. Select "Require external ID" and enter the External ID provided by StarData. This helps prevent the confused deputy problem.
...
14. Share this Role ARN with your StarData representative to complete the setup. StarData will configure their systems to assume this role when accessing your data.
