...
## Overview
[Azure Blob Storage (ABS)](https://learn.microsoft.com/en-us/azure/storage/blobs/storage-blobs-introduction) is a scalable, fully managed, and highly reliable object storage solution offered by Microsoft Azure, designed to store and access data from anywhere in the world. It provides a secure and cost-effective way to store data, including common storage formats such as CSV and Parquet. StarData supports connecting to and reading from Azure Blob Storage using the following Resource URI syntax:
...
When you add data from Azure Blob Storage through the StarData UI, you'll see three authentication options:
...
Storage Account Key credentials provide reliable authentication for Azure Blob Storage. This method works for both local development and StarData Cloud deployments.
...
azure_storage_account: stardatatest
...
For local development, you can use credentials from the Azure CLI. This method is **not suitable for production** or StarData Cloud deployments. This method is only available through manual configuration, and you don't need to create a connector file.
...
StarData will automatically detect and use your local Azure CLI credentials when no connector is specified.

:::warning
This method only works for local development. Deploying to StarData Cloud with this configuration will fail because the cloud environment doesn't have access to your local credentials. Always use Storage Account Key, Connection String, or SAS tokens for production deployments.
:::
