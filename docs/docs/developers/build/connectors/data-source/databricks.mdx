...
[Databricks](https://docs.databricks.com) is a unified data and AI platform built on top of Apache Spark and the lakehouse architecture. StarData connects to Databricks SQL warehouses using the [Databricks SQL Go driver](https://github.com/databricks/databricks-sql-go) to ingest data into StarData's embedded engine.

## Authentication

StarData authenticates to Databricks using a [personal access token](https://docs.databricks.com/aws/en/dev-tools/auth/pat). The token must have access to the SQL warehouse and to the catalogs and schemas you intend to query.
