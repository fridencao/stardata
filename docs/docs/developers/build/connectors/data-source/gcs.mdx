...
When you add data from GCS through the StarData UI, you'll see three authentication options:
...
Service Account JSON credentials provide the most secure and reliable authentication for GCS. This method works for both local development and StarData Cloud deployments.
...
For local development, you can use credentials from the Google Cloud CLI. This method is **not suitable for production** or StarData Cloud deployments. This method is only available through manual configuration, and you don't need to create a connector file.
...
StarData will automatically detect and use your local Google Cloud CLI credentials when no connector is specified.

:::warning
This method only works for local development. Deploying to StarData Cloud with this configuration will fail because the cloud environment doesn't have access to your local credentials. Always use Service Account JSON or HMAC keys for production deployments.
:::
...
2. You will need your Google Cloud project ID to complete this tutorial. Run the following command to show it:
    ...
    ```
3. Replace `[PROJECT_ID]` with your project ID in the following command, and run it to create a new service account (optionally also replace `stardata-service-account` with a name of your choice):
    ```bash
    gcloud iam service-accounts create stardata-service-account --project [PROJECT_ID]
    ```
...
5. Replace `[PROJECT_ID]` with your project ID in the following command, and run it to create a key file for the service account:
    ```bash
    gcloud iam service-accounts keys create stardata-service-account.json \
        --iam-account stardata-service-account@[PROJECT_ID].iam.gserviceaccount.com
    ```
6. You have now created a JSON key file named `stardata-service-account.json` in your current working directory.
