...
description: Power StarData dashboards using BigQuery
sidebar_label: Google BigQuery
sidebar_position: 13
---

[Google BigQuery](https://cloud.google.com/bigquery/docs) is a fully managed, serverless data warehouse that enables scalable and cost-effective analysis of large datasets. StarData supports connecting to BigQuery as a live connector, allowing you to build metrics views and dashboards directly on top of existing BigQuery tables — no data movement required.

:::tip BigQuery as a Live Connector vs. Data Source Connector
StarData supports BigQuery in two distinct modes:

- **OLAP (Live Connector)** — StarData queries BigQuery directly at dashboard load time. No data is ingested into StarData. Use this when your data is already modeled and optimized in BigQuery and you want StarData as a visual layer on top. Set `olap_connector: bigquery` in `rill.yaml`.

- **Data Source Connector** — StarData extracts data from BigQuery and ingests it into its embedded engine (DuckDB or ClickHouse). Use this when you want StarData to manage the data pipeline, apply transformations via SQL models, or combine BigQuery data with other sources. See the [BigQuery data source docs](/developers/build/connectors/data-source/bigquery).
...
For local development, you can omit `google_application_credentials` and StarData will use your local Google Cloud CLI credentials:
...
:::warning Not suitable for StarData Cloud
Application Default Credentials only work locally. When deploying to StarData Cloud, always provide a service account JSON via `google_application_credentials`.
:::
...
:::note
StarData supports metrics views directly on BigQuery as a live connector. Incremental models and partitioned ingestion are not supported in live connector mode.
:::
