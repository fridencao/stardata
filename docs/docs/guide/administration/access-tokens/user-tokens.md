...
User tokens (also called personal access tokens or PATs) provide programmatic access to StarData Cloud tied to your personal user account. They inherit your user permissions and are ideal for local development, scripting, and integrations like MCP (Model Context Protocol).

## Overview

User tokens are designed for:
- **Local development** - Testing and developing with StarData APIs from your machine
...
- **Experimentation** - Trying out StarData APIs without production concerns
- **CLI authentication** - Authenticating StarData CLI commands
...
```bash
stardata token issue --display-name "Local Development"
```
...
stardata token issue --display-name "MCP Token" --ttl-minutes 1440

# Expires in 7 days (10080 minutes)
stardata token issue --display-name "Testing Token" --ttl-minutes 10080
```
...
stardata_usr_abc123...                   Local Development       2024-01-15 10:30     Never
stardata_usr_def456...                   MCP Token              2024-01-16 14:20     2024-01-17 14:20
stardata_usr_ghi789...                   Testing Token          2024-01-14 09:00     2024-01-21 09:00
```
...
```bash
stardata token revoke <token-id>
```

Or revoke by display name:

```bash
stardata token revoke --display-name "Local Development"
```
...
User tokens can be used to authenticate requests to StarData's custom APIs:
