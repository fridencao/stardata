...
description: Create and manage service tokens for programmatic access to StarData
sidebar_label: Service Tokens
sidebar_position: 27
---

Service tokens (also called service accounts) provide programmatic access to StarData Cloud for production systems, scheduled jobs, backend APIs, and other automated workflows. Unlike [user tokens](user-tokens), service tokens persist even if the creating user is removed from the organization.

## Overview

Service tokens are designed for:
- **Production integrations** - Backend services that need to access StarData APIs
- **Scheduled jobs** - Automated reports, data syncs, or ETL processes
- **CI/CD pipelines** - Automated testing and deployment workflows
- **Custom applications** - Applications that integrate with StarData APIs
...
```bash
stardata service create my-service --org-role admin
```

Or with a project-level role:

```bash
stardata service create my-service --project my-project --project-role viewer
```
...
```bash
stardata service create my-service \
  --org-role admin \
...
stardata service create reporting-service \
  --org my-org \
...
Access token: stardata_svc_[TOKEN_HERE]
```
...
```bash
stardata service list --org my-org
```
...
```bash
# Change the name
stardata service edit my-service --new-name renamed-service

# Update attributes
stardata service edit my-service \
  --attributes '{"department":"finance","region":"eu-west"}'

# Clear attributes
stardata service edit my-service --attributes '{}'
```
...
```bash
stardata service set-role my-service --org-role editor
```

Add or update project role:

```bash
stardata service set-role my-service \
  --project analytics-dashboard \
  --project-role admin
```
...
```bash
stardata service show my-service --org my-org
```
...
```bash
stardata service delete my-service --org my-org
```
...
The response contains a short-lived JWT token that can be used to access StarData APIs on behalf of the user.
